...
def main():
    ...
    train_data = LoadData(data_path="vector_data/json/", num_nodes=1024, train_mode="train", graph_type="distance")
    train_loader = DataLoader(train_data, batch_size=128, shuffle=True, num_workers=32)  # num_workers specifies the number of threads for loading data (batch size)

    test_data = LoadData(data_path="vector_data/json/", num_nodes=1024, train_mode="test", graph_type="distance")
    test_loader = DataLoader(test_data, batch_size=128, shuffle=False, num_workers=32)

    # Step 2: Define the model (Assuming the model definition is done separately)
    my_net = GCN(in_c=3, hid_c=8, out_c=16)  # Load GCN model
    result_file = "GCN.h5"
    savefig = "GCN"
    model_path = "gcn"
    # net = "GCN"  # GATNet
    ...
    # net = "ChebNet"  # GATNet
    if net == "ChebNet":
        my_net = ChebNet(in_c=3, hid_c=8, out_c=16, K=2)  # Load ChebNet model
        result_file = "CN.h5"
        savefig = "CN"
        model_path = "cn"
    elif net == "GATNet":
        my_net = GATNet(in_c=3, hid_c=4, out_c=4, n_heads=4)  # Load GAT model
        result_file = "GAT.h5"
        savefig = "GAT"
        model_path = "gat"
    ...
    for epoch in range(Epoch):
        ...
        plt.xlim(2, 200)
        plt.ylim(0, 1)
        plt.savefig("log/plt/" + savefig + "_" + time.strftime("%Y%m%d%H", time.localtime(time.time())) + ".png",
                    bbox_inches='tight')
        plt.show()
