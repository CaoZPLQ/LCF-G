...
def delete_zero(data):
    ...
    for k in data:
        ...
        if len(Node_corrds)>=4:
            puredata[k]=data[k]
    return puredata
def get_feature_vertices_data(input_dir):
    """
    ...
    :return:
        np.array(N,T,D)
    """
    with open('path/to/adjacencies.pkl', 'rb') as f:
        adjacencies = dill.load(f)
    with open('path/to/feature_vertices_data_list.pkl', 'rb') as f:
        feature_vertices_data_list = dill.load(f)
    with open('path/to/labels.pkl', 'rb') as f:
        labels = dill.load(f)
...
class LoadData(Dataset):
    def __init__(self,data_path,num_nodes,train_mode,graph_type):
        """
        ...
        self.feature_vertices_norm,self.feature_vertices_data,self.labels,self.adjacencies=self.pre_process_data(data=get_feature_vertices_data(data_path),norm_dim=1,train_mode=train_mode)
...
if __name__=='__main__':
    train_data=LoadData(data_path= "vector_data/no_pro_json_feature/",num_nodes=1024,train_mode="train",graph_type="distance")
